...
files = ['chelm', 'MountEverest', 'SpacerniakGdansk',
         'WielkiKanionKolorado', 'Redlujjj']
...
for filename in files:
    ...
    fig.suptitle(
        'Interpolacja metodą funkcji sklejanych\'a. Zbiór danych %s' % filename)
    ...
    for i, nodes in enumerate(nodes_number[method_name]):
        ...
        ax.plot(np.array(data['distance']), np.array(
            data['height']), 'x', label='Wartości rzeczywiste')
    ...
    fig.savefig(plots_path + method_name + '_' +
                filename + '.png', bbox='tight')
